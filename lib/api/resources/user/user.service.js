...
var ConsumerService = _interopRequireWildcard(
  require("../../modules/consumer-service.js")
);

var AuthService = _interopRequireWildcard(
  require("../../modules/auth-service.js")
);
...
var _userAccountEvent = _interopRequireDefault(
  require("../../events/user-account-event.js")
);
...
const loginMultiOutletOwner = async ({ params }) => {
  if (!params) {
    return Promise.reject({
      statusCode: _status.BAD_REQUEST,
      message: "request body is required",
  ...
  try {
    ...
    try {
      ...
      _logger.default.error(
        `An error occurred while fetching user details login ${e}`
      );
...
const validateEmail = async (params) => {
  ...
  try {
    ...
    _logger.default.error(
      "An error occurred while verifying OTP sent to email"
    );
...
const requestResetPassword = async ({ params }) => {
  ...
  try {
    ...
    _logger.default.error(
      "An error occurred when requesting for password reset"
    );
